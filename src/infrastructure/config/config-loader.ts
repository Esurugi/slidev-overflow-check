...
export class ConfigLoader {
  ...
  private normalizeConfig(config: any): Partial<CheckerOptions> {
    ...
    if (config.failOnIssues !== undefined) normalized.failOnIssues = config.failOnIssues;
    if (config.concurrency !== undefined) normalized.concurrency = config.concurrency;
    if (config.contentAnalysis) normalized.contentAnalysis = config.contentAnalysis;
